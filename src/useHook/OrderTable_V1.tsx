...
export default function OrderTable() {
  ...
  return (
    <>
      ...
      {/* 平板以上搜尋與過濾 */}
      <Box sx={{ borderRadius: 'sm', py: 2, display: { xs: 'none', sm: 'flex' }, flexWrap: 'wrap', gap: 1.5, '& > *': { minWidth: { xs: '120px', md: '160px' } } }}>
      ...
      {/* 表格 */}
      <Sheet variant="outlined" sx={{ display: { xs: 'none', sm: 'initial' }, width: '100%', borderRadius: 'sm', overflow: 'auto', minHeight: 0 }}>
        <Table stickyHeader hoverRow sx={{ '--TableCell-paddingY': '4px', '--TableCell-paddingX': '8px' }}>
          <thead>
            <tr>
              ...
              <tr key={row.id}>
                <td style={{ textAlign: 'center' }}>
                  <Checkbox
                    ...
                    onChange={(event) => setSelected(ids => event.target.checked ? ids.concat(row.id) : ids.filter(id => id !== row.id))}
                  />
                </td>
                <td><Typography level="body-xs">{row.id}</Typography></td>
                <td><Typography level="body-xs">{row.project_name}</Typography></td>
                <td>
                  <Chip
                    variant="soft"
                    size="sm"
                ...
                <td><Typography level="body-xs">{row.execution_info.planned_progress}</Typography></td>
                <td><Typography level="body-xs">{row.execution_info.actual_progress}</Typography></td>
                <td><Typography level="body-xs">{row.execution_info.execution_content}</Typography></td>
              </tr>
            ))}
          </tbody>
        </Table>
      ...
      {/* 分頁 */}
      <Box sx={{ pt: 2, gap: 1, [`& .${iconButtonClasses.root}`]: { borderRadius: '50%' }, display: { xs: 'none', md: 'flex' } }}>
        <Button size="sm" variant="outlined" color="neutral" startDecorator={<KeyboardArrowLeftIcon />}>Previous</Button>
        <Box sx={{ flex: 1 }} />
        {['1', '2', '3', '…', '8', '9', '10'].map(page => (
          <IconButton key={page} size="sm" variant={Number(page) ? 'outlined' : 'plain'} color="neutral">{page}</IconButton>
        ))}
        <Box sx={{ flex: 1 }} />
